...
public class Joint3Cmd  extends Command {
	...
	public Joint3Cmd(int currentPosition, int setPosition, int time) {
		...
		setPosition = Math.min(setPosition, Robot.JOINT3_HLM);
		setPosition = Math.max(setPosition, Robot.JOINT3_LLM);
		setPos = setPosition;
		stepNum = time/20;
	...
	protected boolean isFinished() {
		if (stepCnt > stepNum + TIMEOUT_TH) {
			System.out.println("On target 3 by time out 1 second");
			return true;
		}else if ((stepCnt > stepNum) && Robot.joint3Subsystem.onTarget()) {
			//Robot.joint3Subsystem.disable();  //for arm maybe we could not disable the PID to keep arm at position
			System.out.println("On target 3");
			return true;
		}else {
